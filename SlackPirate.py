        ...
        for channel_name, channel_id in channel_list.items():
            while True:
                ...
                if is_rate_limited(r):
                    continue
                channel_pinned_messages = [m.get('text') for m in r.get('items', []) if m.get('type') == 'message']
    ...
    if pinned_message_contents:
        with open(output_file, 'a', encoding="utf-8") as out:
            for text_content in pinned_message_contents:
                out.write(channel_name + "{}, {}\n".format(channel_name, text_content))
        print(termcolor.colored(
            "END: If any pinned messages were found, they will be here: ./{}".format(output_file), "white", "on_green"))
    else:
